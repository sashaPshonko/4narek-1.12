...
const clanOwnerWatch = createClanOwnerBanWatch({ anarchy: 503, rootDir: __dirname });
...
async function initTelegram() {
    ...
    try {
        await tgBot.sendMessage(alertChatID, '✅ Оркестратор 503 запущен');
    } catch (error) {
...
startLogRotate([
    join(__dirname, 'logs/503.log'),
    join(__dirname, '503.log'),
