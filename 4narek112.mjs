...
async function launchBookBuyer(name, password, anarchy) {
    ...
    bot.once('spawn', () => {
        ...
        watchdogTimer = setInterval(() => {
            if (watchdogBusy || walkInProgress || afkHandlerBusy) return;

            const idle = Date.now() - botTimeActive;
            const sellStuck = mu && idle >= IDLE_HARD_MS;
            const idleTooLong = !mu && idle >= IDLE_SOFT_MS;
            if (!sellStuck && !idleTooLong) return;
            ...
            (async () => {
                try {
                    if (sellStuck) {
                        ...
                        await rnd(DELAY.CHAT);
                        await antiAfkMovement(bot, 4000);
                        bot.chat(anarchyCommand);
                        await delay(400);
                        botTimeLogin = Date.now();
                    } else {
                        logger.info(`${name} - watchdog: простой ${Math.floor(idle / 1000)}с`);
...
//
        ...
        if (messageText.includes('Данная команда недоступна в режиме AFK')) {
            void handleAfkMessage(bot, name);
            return;
        }
...
async function handleAfkMessage(bot, name) {
    if (afkHandlerBusy) return;
    afkHandlerBusy = true;

    try {
        logger.info(`${name} - AFK, стоп продажи → аукцион`);
        sellAfkAbort = true;
        mu = false;
        generateRandomKey(bot);
...
async function sellItems(bot, itemPrices, botName = '') {
    ...
    try {
        ...
        if (sellAfkAbort) {
            ...
            mu = false;
            logger.info(`${bot.username} - продажа прервана (AFK)`);
            return;
        }
