...
const clanOwnerWatch = createClanOwnerBanWatch({ anarchy: 504, rootDir: __dirname });
...
async function initTelegram() {
    ...
    try {
        await tgBot.sendMessage(alertChatID, '✅ Оркестратор 504 запущен');
    } catch (error) {
...
startLogRotate([
    join(__dirname, 'logs/504.log'),
    join(__dirname, '504.log'),
