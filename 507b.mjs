...
const clanOwnerWatch = createClanOwnerBanWatch({ anarchy: 507, rootDir: __dirname });
...
// ========== КОНСТАНТЫ ==========
const botsPath = join(__dirname, './bots/507b.json');
const token = '8769142111:AAH8T7q9lQGf4i7sLlpF1oM7TZodoWwuxjk';
        ...
        try {
            ...
            worker.on('message', async (message) => {
                try {
                    ...
                    } else if (message.name === 'ah_lots') {
                        forwardAhLotsToGo(socket, isSocketOpen, message.lots);
...
async function initTelegram() {
    ...
    try {
        await tgBot.sendMessage(alertChatID, '✅ Оркестратор 507 запущен');
    } catch (error) {
