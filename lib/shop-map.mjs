...
/**
 * Можно ли пройти по прямой a→b, не свалившись.
 *
 * Проверяем не центр шага, а углы хитбокса: бот шириной ~0.6, и по диагонали
 * он задевает соседние клетки — именно так и происходит падение в щель.
 */
export function segmentWalkable(cells, a, b, { half = 0.35 } = {}) {
    const dx = b.x - a.x;
    const dz = b.z - a.z;
    ...
    for (let i = 1; i <= steps; i++) {
        ...
        for (const ox of [-half, half]) {
            for (const oz of [-half, half]) {
                const c = cells[cellKey(Math.round(px + ox), Math.round(pz + oz))];
                if (!c) return false;
...
export function replanLeg(cells, fromPos, waypoint, {
    ...
    return simplifyPathSafe(cells, path).slice(1);
}
