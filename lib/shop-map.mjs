...
const DEFAULTS = {
    ...
    freshBonus: 30,
    /** shop почти плоский — узкое окно Y сильно режет blockAt */
    yScanDown: 5,
    yScanUp: 4,
};
...
/** Сколько сторон открыто — у стены 1–2, в проходе 3–4. */
function openSides(cells, x, z) {
    let n = 0;
    for (const [nx, nz] of neighbors4(x, z)) {
        if (cells[cellKey(nx, nz)]) n++;
    }
    return n;
}

/** Пол, на который бот заходит шагом, без прыжка. */
function isWalkUp(floor) {
    const n = String(floor || '');
    return n.includes('stairs') || n.includes('slab') || n.includes('carpet');
}

/**
 * Можно ли перейти из a в b, ни во что не упёршись.
 *
 * Раньше пускали любой перепад ≤1 в обе стороны. Но подъём на целый блок бот
 * пешком не берёт: он давит в стенку, пока его не выбьет jump unstick — это и
 * есть «упёрся в блок». Наверх пускаем только туда, куда заходят шагом
 * (ступени, плиты); вниз с блока можно спрыгнуть всегда.
 */
function canStepFlat(a, b) {
    if (!a || !b) return false;
    const dy = b.y - a.y;
    if (dy < -1.01) return false;
    if (dy <= 0.01) return true;
    return dy <= 1.01 && isWalkUp(b.floor);
}

/** Проход, по которому бот пройдёт в обе стороны — иначе карта ловит в яму. */
function canStepBoth(a, b) {
    return canStepFlat(a, b) && canStepFlat(b, a);
}
...
/**
 * Можно ли шагнуть из мировой точки в клетку (ix,iz) по карте.
 * Нет клетки = стена / яма / зона — туда не идём.
 */
export function mapStepOk(cells, from, ix, iz) {
    const next = cells?.[cellKey(ix, iz)];
    if (!next) return false;
    const cur = cells[cellKey(blockIndex(from.x), blockIndex(from.z))]
        || { x: blockIndex(from.x), z: blockIndex(from.z), y: from.y, floor: '' };
    return canStepFlat(cur, next);
}
...
export function astarAvoiding(cells, start, goal, { blocked = null } = {}) {
    ...
    while (open.size) {
        ...
        for (const [nx, nz] of neighbors8(curCell.x, curCell.z)) {
            const nk = cellKey(nx, nz);
            const next = cells[nk];
            if (!next || closed.has(nk)) continue;
            if (blocked?.has(nk)) continue;
            if (!canStepFlat(curCell, next)) continue;
            const diag = nx !== curCell.x && nz !== curCell.z;
            if (diag) {
                // диагональ только если оба ортогональных угла свободны —
                // иначе путь режет угол и хитбокс садится в блок
                const sideA = cells[cellKey(nx, curCell.z)];
                const sideB = cells[cellKey(curCell.x, nz)];
                if (!sideA || !sideB) continue;
                if (!canStepFlat(curCell, sideA) || !canStepFlat(curCell, sideB)) continue;
            }
            const step = diag ? 1.414 : 1;
            // у стены дороже: бот шириной 0.6, клетка вплотную к блоку — это касание
            const hug = Math.max(0, 3 - openSides(cells, nx, nz)) * 0.45;
            const tentative = baseG + step + hug;
    ...
    const grow = (startK) => {
        ...
        for (let qi = 0; qi < q.length; qi++) {
            ...
            for (const [nx, nz] of neighbors4(cur.x, cur.z)) {
                ...
                if (!canStepBoth(cur, pool[nk])) continue;
                seen.add(nk);
                q.push(nk);
            }
        }
 ...
 */
// половина ширины бота — чуть меньше реальных 0.3, иначе тот, кто идёт
// вплотную к стене, сам себе кажется застрявшим и маршрут дёргается
export function segmentWalkable(cells, a, b, { half = 0.29 } = {}) {
    const dx = b.x - a.x;
    const dz = b.z - a.z;
    ...
    for (let i = 1; i <= steps; i++) {
        ...
        for (const ox of [-half, half]) {
            for (const oz of [-half, half]) {
                ...
                if (!c) return false;
                const dy = c.y - prevY;
                if (dy < -1.01 || dy > 1.01) return false;
                // на срезе тоже нельзя упереться в ступеньку высотой в блок
                if (dy > 0.01 && !isWalkUp(c.floor)) return false;
...
export async function dumpShopFlatMap(bot, {
    ...
    if (MAP_SCAN_DISABLED) {
        log?.('shop-map → dump отключён (MAP_SCAN_DISABLED)');
        throw new Error('map_scan_disabled');
    }
    ...
    const dump = await dumpWalkableGrid(bot, {
        radiusXZ,
        log: logFn,
        preferY: here.y,
        yScanDown: DEFAULTS.yScanDown,
        yScanUp: DEFAULTS.yScanUp,
        yieldEveryRows: 2,
        circle: true,
    });
...
export async function ensureShopFlatMap(bot, {
    ...
    if (MAP_SCAN_DISABLED) {
        if (existsSync(SHOP_DUMP_REQUEST_PATH)) {
            log?.('shop-map → dump-request игнор (MAP_SCAN_DISABLED)');
            try {
        ...
        if (existing && existing.walkableCount >= minFlatCells) {
            log?.(
                `shop-map → только кэш (скан выкл): клеток=${existing.walkableCount}`,
            );
            return existing;
        }
        throw new Error('map_scan_disabled_no_cache');
    }
...
export function pickSpreadGoal(cells, from, {
    ...
    /** mineflayer yaw: сильно предпочитаем цели впереди (без поворота головы) */
    facingYaw = null,
    rng = Math.random,
} = {}) {
    ...
    const face = Number.isFinite(facingYaw)
        ? { x: -Math.sin(facingYaw), z: -Math.cos(facingYaw) }
        : null;

    let best = null;
    let bestScore = -Infinity;
    const tries = Math.min(320, all.length);
    for (let t = 0; t < tries; t++) {
        ...
        if (face) {
            ...
            const dz = c.z - from.z;
            const len = Math.hypot(dx, dz) || 1;
            const dot = face.x * (dx / len) + face.z * (dz / len);
            // сзади почти не берём; впереди — жирный бонус
            if (dot < -0.15) faceScore = -60;
            else if (dot < 0.25) faceScore = dot * 20;
            else faceScore = 25 + dot * 35;
        }

        const score = sectorBonus + edgeBonus + distScore + leaveHub + freshScore + faceScore + rng() * 4;
        if (score > bestScore) {
            bestScore = score;
            best = c;
        }
    ...
    if (!best) {
        for (const c of all) {
            ...
            let score = Math.hypot(c.x - hub.x, c.z - hub.z) + rng();
            if (face) {
                const dx = c.x - from.x;
                const dz = c.z - from.z;
                const len = Math.hypot(dx, dz) || 1;
...
export function replanLeg(cells, fromPos, waypoint, {
    ...
    // первой точкой оставляем центр клетки, в которой бот стоит: если его
    // прижало к краю, он сначала выходит на середину прохода, а не режет угол
    return simplifyPathSafe(cells, path.map(cellPoint));
}
