 ...
 *
 * Окно из трёх блоков едет вниз: floor текущего шага — это feet следующего.
 * Один blockAt на уровень вместо трёх, иначе радиус 110 не влезает.
 */
function probeStandAt(bot, x, z, preferY, {
    ...
    const yMin = y0 - yScanDown;
    const yMax = y0 + yScanUp;
    const candidates = [];
    let feet = blockAt(bot, x, yMax + 1, z);
    let head = blockAt(bot, x, yMax + 2, z);
    for (let y = yMax; y >= yMin; y--) {
        const floor = blockAt(bot, x, y, z);
        if (isStandableFloor(floor) && isAirLike(feet) && isAirLike(head)) {
            const feetY = y + 1;
            candidates.push({
                y: feetY,
                floorY: y,
 ...
 * }}
 */
/** Live-скан нужен для shop_map walk (кэша shop-map.json нет). yieldEveryRows в dumpWalkableGrid. Offline — scripts/rebuild-walk-route.mjs */
export const MAP_SCAN_DISABLED = false;

export async function dumpWalkableGrid(bot, {
    radiusXZ = DEFAULTS.radiusXZ,
    preferY = null,
    log = null,
    /** чаще yield — иначе r≈100 залипает event loop на секунды */
    yieldEveryRows = 2,
    yScanDown = DEFAULTS.yScanDown,
    yScanUp = DEFAULTS.yScanUp,
    /** квадрат с кругом: −21% клеток без потери покрытия территории */
    circle = true,
} = {}) {
    if (MAP_SCAN_DISABLED) {
        log?.('spawn-map → dump отключён (MAP_SCAN_DISABLED)');
    ...
    const probeOpts = { yScanDown, yScanUp };

    // За краем прогрузки blockAt всегда null — пробуем чанк один раз, иначе
    // большой радиус тратит сотни тысяч вызовов на пустоту.
    const chunkSeen = new Map();
    const chunkLoaded = (x, z) => {
        const cx = Math.floor(x / 16);
        const cz = Math.floor(z / 16);
        const k = `${cx},${cz}`;
        let hit = chunkSeen.get(k);
        if (hit === undefined) {
            hit = blockAt(bot, cx * 16 + 8, Math.floor(oy), cz * 16 + 8) != null;
            chunkSeen.set(k, hit);
            if (!hit) skippedChunks++;
        }
        return hit;
    };
    ...
    for (let dx = -radiusXZ; dx <= radiusXZ; dx++) {
        for (let dz = -radiusXZ; dz <= radiusXZ; dz++) {
            if (circle && (dx * dx + dz * dz) > r2) continue;
            scanned++;
            const x = ox + dx;
            const z = oz + dz;
            if (!chunkLoaded(x, z)) continue;
            const stand = probeStandAt(bot, x, z, oy, probeOpts);
            if (!stand) continue;
            cells[key(x, z)] = {
                x,
                z,
        ...
        // скан идёт в живом клиенте: залипший loop сам по себе выглядит как краш
        if (yieldEveryRows > 0 && (dx + radiusXZ) % yieldEveryRows === 0) {
    ...
    log?.(
        ...
        + (skippedChunks ? ` (не прогружено чанков: ${skippedChunks})` : ''),
    );
...
export async function maybeDumpSpawnMap(bot, log = console.log) {
    if (MAP_SCAN_DISABLED) {
        if (existsSync(DUMP_REQUEST_PATH)) {
            log?.('spawn-map → dump-request игнор (MAP_SCAN_DISABLED)');
        }
        return false;
    }
    ...
    try {
        ...
        const dump = await dumpWalkableGrid(bot, { log: logFn });
