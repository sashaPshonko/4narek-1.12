...
export async function handleCaptchaLogin(bot, opts) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        log(`attempt ${attempt}/${maxAttempts} capture…`);
        const captured = await waitForCaptcha(bot, {
            quietMs: opts.quietMs ?? 900,
            maxWaitMs: opts.maxWaitMs ?? 20_000,
            prevFingerprint,
            // FunTime после wrong почти не шлёт новые map-пакеты (reuse id) —
            // requireFreshMaps=true зависает на stale. Ждём смену fingerprint.
        ...
        if (verdict === 'wrong') {
            log(`wrong answer ${solved.pred}, retry…`);
            await sleep(600);
            continue;
        }
