...
/** Кэш map-байтов на боте: FunTime после wrong часто не шлёт map снова. */
export function attachMapCache(bot) {
    ...
    bot._client.on('map', (packet) => {
        ...
        bot._kapchaMapCache.set(Number(id), Buffer.from(packet.data));
    });
...
export function waitForCaptcha(bot, opts = {}) {
    ...
    // По умолчанию false: после wrong сервер часто не шлёт map-пакеты (reuse id).
    // Кашу ловим QC по швам + сменой fingerprint (rotate/coords/байты).
    ...
    return new Promise((resolve) => {
        ...
        const finish = async (reason) => {
            ...
            if (requireFreshMaps) {
                const stale = best.filter((f) => !freshMapCounts.has(f.mapId));
                if (stale.length) {
                    log(
