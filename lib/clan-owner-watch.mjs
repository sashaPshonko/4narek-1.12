...
/** Первый пинг: 5–25 мин + сдвиг по анке (орхи не стартуют owner разом). */
function firstDelayMs(an) {
    const anNum = Number(an) || 0;
    return (5 + rndInt(0, 20)) * 60_000 + (anNum % 5) * 7 * 60_000;
}

/** Между пингами: 40–75 мин + сдвиг по анке (не общий ритм 502/504). */
function nextIntervalMs(an) {
    const anNum = Number(an) || 0;
    return (40 + rndInt(0, 35)) * 60_000 + (anNum % 7) * 3 * 60_000;
}
...
export function createClanOwnerBanWatch({ anarchy, rootDir }) {
    ...
    function scheduleNext(ms, log = console.log) {
        ...
        timer = setTimeout(() => {
            void (async () => {
                ...
                log(`[clan-owner-watch] an${an} next ping in ~${Math.round(next / 60000)}м`);
    ...
    function start(hooks) {
        ...
        log(
            `[clan-owner-watch] an${an} стартовал — первый ping через ~${Math.round(first / 60000)}м`
            + ` (далее 40–75м random + сдвиг анки, через launch-gate)`,
        );
