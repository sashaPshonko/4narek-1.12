...
export async function runAntiAfkMotion(bot, log = console.log, shouldAbort = null, _opts = null) {
    ...
    if (lookSteer) {
        ...
        // continuous W у края/стены иногда даёт walked=0 — добираем классическим WASD
        if (walked < 1.5 && !(typeof shouldAbort === 'function' && shouldAbort())) {
            const logFn = typeof log === 'function' ? log : console.log;
            logFn('anti-AFK → fallback WASD после look-steer');
