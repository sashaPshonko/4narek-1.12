...
export async function runAntiAfkMotion(bot, log = console.log, shouldAbort = null, _opts = null) {
    ...
    if (lookSteer) {
        ...
        const walked = await antiAFKMoveForwardLook(bot, log, shouldAbort, {
            short: true,
            label: 'anti-AFK',
        });
        // почти не сдвинулись — один короткий WASD-ключ, не полный 5с бурст
        if (walked < 0.8 && !(typeof shouldAbort === 'function' && shouldAbort())) {
            const logFn = typeof log === 'function' ? log : console.log;
            logFn('anti-AFK → mini-WASD после short look-steer');
            await antiAFKMoveShort(bot, logFn, shouldAbort);
        }
        return;
    }
