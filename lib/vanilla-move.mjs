 ...
 * - hasHorizontalCollision = bot.entity.isCollidedHorizontally (prismarine)
 */
...
function applyWalkPatch(bot) {
    ...
    const fixMoveFlags = (params) => {
        ...
        // prismarine-physics выставляет isCollidedHorizontally каждый simulatePlayer
    ...
    client.write = (name, params) => {
        // Sprint не используем — не пускаем start_sprinting наружу.
    ...
    bot.setControlState = function setControlStateVanilla(control, state) {
        ...
        // Sprint off: не включаем и не зовём orig(true) → нет entity_action.
        if (control === 'sprint') {
            if (state) return;
    ...
    // НЕ шлём tick_end на каждый physicsTick: FunTime после такого флуда
    // принимает /clan, но глушит /ah,/balance,/warp (окно не открывается).
    // tick_end остаётся только после position/look (queueTickEnd выше).
}
