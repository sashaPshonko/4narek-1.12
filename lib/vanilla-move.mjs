/**
 * Anti-AFK: WASD-бурст по умолчанию; на 502 / lookSteer — forward+мышь
 ...
 *   lookSteer / anarchy===502 → forward+мышь; иначе классический WASD-бурст.
 */
export async function runAntiAfkMotion(bot, log = console.log, shouldAbort = null, _opts = null) {
    ...
    const lookSteer = opts?.lookSteer === true
        || Number(opts?.anarchy) === 502
        || process.env.AFK_LOOK_STEER === '1';
    if (lookSteer) {
        await antiAFKMoveForwardLook(bot, log, shouldAbort);
        return;
    }
...
function applyWalkPatch(bot, {
    ...
    client.write = (name, params) => {
        ...
        if (MOVE_PACKET_NAMES.has(name) && shouldTouchMove()) {
            ...
            const out = rawWrite(name, fixMoveParams(params));
