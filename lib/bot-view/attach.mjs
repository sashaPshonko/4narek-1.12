...
function sendPlayerPosition(client, bot, cache, log) {
    if (cache.position) {
...
function sendWorld(client, bot, cache, log) {
    ...
    const chunks = all
        .filter((p) => p?.raw && packetId(p.raw) === 0x2c)
        .sort((a, b) => {
            const da = Math.max(Math.abs((a.x ?? 0) - cx), Math.abs((a.z ?? 0) - cz));
            const db = Math.max(Math.abs((b.x ?? 0) - cx), Math.abs((b.z ?? 0) - cz));
            return da - db;
        });
    const skipped = all.length - chunks.length;

    // viewDistance=8 → клиент ждёт ~289 чанков; у нас кэш ~75 → «загрузка территории» висит.
    let maxRing = 2;
    for (const p of chunks) {
        const d = Math.max(Math.abs((p.x ?? 0) - cx), Math.abs((p.z ?? 0) - cz));
        if (d > maxRing) maxRing = d;
    }
    const viewDistance = Math.max(2, Math.min(6, maxRing));

    writeSafe(client, 'game_state_change', { reason: 'level_chunks_load_start', gameMode: 0 });
    writeSafe(client, 'update_view_distance', { viewDistance });
    writeSafe(client, 'update_view_position', { chunkX: cx, chunkZ: cz });
    // позицию до чанков — иначе Loading terrain ждёт весь dump
    sendPlayerPosition(client, bot, cache, log);

    if (chunks[0]?.raw) {
        log(`chunk0 id=${packetId(chunks[0].raw)} len=${chunks[0].raw.length} hex=${chunks[0].raw.subarray(0, 8).toString('hex')}`);
    }
    ...
    for (let i = 0; i < chunks.length; i += BATCH) {
        ...
        writeSafe(client, 'chunk_batch_finished', { batchSize: Math.max(slice.length, 1) });
    }
    log(`мир: ${n} чанков (пропуск ${skipped}) view=${cx},${cz} vd=${viewDistance}`);
...
export function startAttachServer({
    ...
    getBot,
    onSpectatorChat,
    /** @type {(name: string, data: object, client: object) => void | null} */
    onSpectatorPlayPacket = null,
    log = console.log,
}) {
    ...
    server.on('playerJoin', (client) => {
        ...
        client.on('packet', (data, meta) => {
            ...
            if (typeof onSpectatorPlayPacket === 'function') {
                try {
                    onSpectatorPlayPacket(meta.name, data, client);
                } catch (err) {
                    log(`pilot pkt: ${err?.message || err}`);
                }
            }
        });
