...
export function createPacketCache() {
    ...
    function onPacket(data, meta, buffer, fullBuffer) {
        ...
        if (CHUNK_NAMES.has(name) && data && data.x != null && data.z != null) {
            state.chunks.set(`${data.x},${data.z}`, { name, data: clonePacket(data), raw });
            return;
        }
        if (name === 'map' && data && data.itemDamage != null) {
            state.maps.set(data.itemDamage, { name, data: clonePacket(data), raw });
            return;
        }
        if (name === 'map' && data && data.mapId != null) {
            state.maps.set(data.mapId, { name, data: clonePacket(data), raw });
            return;
        }
        ...
        if (ENTITY_SPAWN_NAMES.has(name)) {
            ...
            if (id != null) state.entities.set(id, { name, data: clonePacket(data), raw });
            return;
        }
        if (name === 'entity_metadata' || name === 'set_entity_metadata') {
            const id = entityIdOf(data);
            if (id != null) state.entityMeta.set(id, { name, data: clonePacket(data), raw });
            return;
        }
    ...
    function onWire(stateName, buf) {
        ...
        if (id === I.PLAY_MAP) {
            state.maps.set(state.maps.size, { name: 'map', raw });
        }
    }
