...
export const WARP_NAMES = ['mine', 'casino', 'portal', 'palach', 'fisher', 'stash', 'wagon'];

/** Аварийный /warp (яма, y-desync, floor-watchdog) — из того же списка. */
export function randomWarpName() {
    const i = Math.floor(Math.random() * WARP_NAMES.length);
    return WARP_NAMES[i] || 'portal';
}

export function randomWarpCmd() {
    return `/warp ${randomWarpName()}`;
}

/** Дневная прогулка: случайный интервал 3–5 мин после прошлой. */
export const WARP_COOLDOWN_BASE_MS = 180_000;
export const WARP_COOLDOWN_JITTER_MS = 120_000;
export const WARP_SKIP_BPS = 400; // ~4% — редко скипаем цикл
...
/** Кулдаун на текущий gap после lastWarpTime (рандом 3–5 мин, фиксируется до следующей прогулки). */
const cooldownByGap = new Map();

function gapKey(username, lastWarpTime) {
    return `${username}:${lastWarpTime || 0}`;
}

/** Случайный интервал 3–5 мин для текущего gap (username + lastWarpTime). */
export function warpCooldownMs(username, lastWarpTime = 0) {
    const key = gapKey(username, lastWarpTime);
    let ms = cooldownByGap.get(key);
    if (!Number.isFinite(ms)) {
        ms = WARP_COOLDOWN_BASE_MS
            + Math.floor(Math.random() * (WARP_COOLDOWN_JITTER_MS + 1));
        cooldownByGap.set(key, ms);
        // не копить вечно
        if (cooldownByGap.size > 500) {
            const first = cooldownByGap.keys().next().value;
            cooldownByGap.delete(first);
        }
    }
    return ms;
}
...
export function shouldAttemptWarp(username, lastWarpTime, workerStartTime, nowMs = Date.now()) {
    ...
    } else if (nowMs - lastWarpTime < warpCooldownMs(username, lastWarpTime)) {
        return false;
    }
