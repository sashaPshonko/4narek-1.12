...
/** Шаг мыши vanilla 100% — дельты поворота обязаны быть кратны ему. */
...
export async function antiAFKdragMouse(bot, log = console.log, shouldAbort = null) {
    ...
    // Сдвиг мыши — целое число шагов чувствительности. bot.look(force=true)
    // отключает родную защиту mineflayer и шлёт сырой угол, а GCD-чек
    // античита ловит именно дробные дельты.
    const finalYaw = currentYaw + Math.round((smoothYaw - currentYaw) / LOOK_GCD_STEP) * LOOK_GCD_STEP;
    const finalPitch = currentPitch + Math.round((smoothPitch - currentPitch) / LOOK_GCD_STEP) * LOOK_GCD_STEP;
    await bot.look(finalYaw, finalPitch, true);
    if (bot.entity) {
        bot.entity.yaw = finalYaw;
        bot.entity.pitch = finalPitch;
    }
    logFn(
        `anti-AFK мышь (${currentYaw.toFixed(2)}, ${currentPitch.toFixed(2)}) → (${finalYaw.toFixed(2)}, ${finalPitch.toFixed(2)})`,
    );
