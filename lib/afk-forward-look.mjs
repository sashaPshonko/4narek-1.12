...
export async function antiAFKMoveForwardLook(bot, log = console.log, shouldAbort = null) {
    ...
    // Сначала смотрим куда идём — иначе W в стену/край → walked=0
    lookInstant(bot, targetYaw, clampPitch(rnd(PITCH_IDLE_MIN, PITCH_IDLE_MAX)));
    await sleep(80);
    if (typeof shouldAbort === 'function' && shouldAbort()) return 0;
        ...
        while (Date.now() < sessionEnd) {
            ...
            if (now >= nextPitCheck) {
                ...
                if (ahead < AHEAD_MIN_SAFE || yawWouldFall(bot, bot.entity.yaw)) {
                    ...
                    if (alt == null) {
                        // ещё не прошли — пусть caller сделает WASD fallback
                        if (walked < 1.0) {
            ...
            if (now - stuckAt >= STUCK_CHECK_MS) {
                ...
                if (slid < STUCK_MOVE_MIN || horizSpeed(bot) < 0.035) {
                    stuckN += 1;
                    // отпустить W → доворот → снова W (иначе упираемся в стену)
                    ...
                    targetYaw = alt;
                    lookInstant(bot, targetYaw, bot.entity.pitch);
                    await sleep(100);
                    holdForward(bot);
