...
/**
 * Anti-AFK / дневная прогулка: press W → долго идём + look на ходу → release W.
...
export async function antiAFKMoveForwardLook(bot, log = console.log, shouldAbort = null, opts = null) {
    ...
    const label = typeof options.label === 'string' ? options.label : 'anti-AFK';
    const zones = options.zones || null;
    const distMin = Number.isFinite(options.totalDistMin) ? options.totalDistMin : TOTAL_DIST_MIN;
    const distMax = Number.isFinite(options.totalDistMax) ? options.totalDistMax : TOTAL_DIST_MAX;
    const sessionMs = Number.isFinite(options.sessionMsMax) ? options.sessionMsMax : SESSION_MS_MAX;
