 ...
 *
 * Как ночной baseline: player_input только при смене setControlState (anti-AFK).
 * Повторный write — только явный bot.refreshPlayerInput() из цикла ходьбы,
 * без пульса каждый physicsTick (тот ломал anti-AFK / FunAC 4.3.1).
 *
 * physicsTick / fallback 50мс — только tick_end.
 *
...
function applyWalkPatch(bot) {
    ...
    /** Только для walk-route: anti-AFK не вызывает. */
