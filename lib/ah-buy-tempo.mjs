/**
 * Задержка клика на АХ: (700–1300 мс) × номер строки.
...
const AH_BUY_STEP = { min: 700, max: 1300 };
...
/** FunTime «подозрительная цена»: подтверждаем покупку, не отмену. */
export function ahSuspiciousConfirmDelayMs() {
    return delayMs({ min: 5000, max: 10000 });
}
