 ...
 * (даже если он один) → дальше browse: reload или листание.
 *
 * Reload: чаще 50, иногда промах на 49.
 * Листание вперёд: 51; следующий reload вернёт на 1-ю страницу.
 */
...
/** Вместо reload — листание вперёд (51). */
const PAGE_FLIP_CHANCE = 0.12;
...
export const AH_SLOT_PAGE_NEXT = 51;
...
/**
 * Действие, когда не покупаем: reload или страница вперёд.
 * @returns {{ type: 'reload'|'page', slot: number }}
 */
export function pickAhBrowseAction() {
    if (Math.random() < PAGE_FLIP_CHANCE) {
        return { type: 'page', slot: AH_SLOT_PAGE_NEXT };
    }
