...
async function walkShopMapSession(bot, route, {
    ...
    const useLookSteer = anNum === 502 || anNum === 504;
    const opts = {
        ...
        zones,
        cells: map.cells,
        // 502/504: continuous W + look к вейпоинту (не strafe-диагонали)
        lookSteer: useLookSteer,
    };
    if (useLookSteer) {
        logFn?.(`walk → lookSteer ON (W+look) an=${anNum}`);
    }
