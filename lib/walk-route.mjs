...
async function walkToPoint(bot, target, opts) {
    ...
    while (Date.now() < deadline) {
        ...
        if (blocked && Date.now() > slideUntil) {
        ...
        const sliding = Date.now() < slideUntil && slideSide;
        const lookX = sliding
...
export async function walkRandomRouteStop(bot, {
    ...
    // Старт как задумано: со спавна (точка 0), если рядом.
    // Уже на треке (близко к вершине) — едем вдоль с её s.
    // Иначе mid-join только если проекция близко.
    let entry;
    ...
    } else if (onTrack.dist <= route.arriveXZ * 2) {
