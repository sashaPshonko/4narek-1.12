...
/**
 * LookSteer: yaw с открытым коридором к цели (не ровно в стену).
 * @returns {number|null}
 */
function pickOpenSteerYaw(bot, from, target, cells, preferYaw = null) {
    const want = yawTo(from.x, from.z, target.x, target.z);
    const base = Number.isFinite(preferYaw) ? preferYaw : want;
    const offsets = [
        0,
        0.18, -0.18, 0.35, -0.35, 0.55, -0.55,
        0.85, -0.85, 1.15, -1.15, 1.55, -1.55,
    ];
    let best = null;
    let bestScore = -Infinity;
    const tdx = target.x - from.x;
    const tdz = target.z - from.z;
    const tlen = Math.hypot(tdx, tdz) || 1;
    const tx = tdx / tlen;
    const tz = tdz / tlen;
    for (const off of offsets) {
        const roots = Math.abs(normalizeAngle(base - want)) > 0.08
            ? [want, base]
            : [want];
        for (const root of roots) {
            const y = normalizeAngle(root + off);
            const dir = lookXZ(y);
            let clearDist = 0;
            let blocked = false;
            for (const d of [0.75, 1.25, 1.85]) {
                if (wallAhead(bot, y, { cells, dist: d, dir })) {
                    blocked = true;
                    break;
                }
                clearDist = d;
            }
            if (blocked && clearDist < 0.75) continue;
            const dot = tx * dir.x + tz * dir.z;
            if (dot < 0.12) continue;
            const score = dot * 2 + clearDist - Math.abs(normalizeAngle(y - want)) * 0.35;
            if (score > bestScore) {
                bestScore = score;
                best = y;
            }
...
/**
 * К цели: по умолчанию WASD без look; lookSteer (502/504) — W + мягкий look.
 * Предпочтение WASD: forward (+strafe) > strafe. Цель сзади → behind (не portal).
 */
async function walkStraightToPoint(bot, target, {
    ...
    let lastLogAt = 0;
    // WASD: курс фиксируем. lookSteer: свой yaw шагами GCD (не snap каждый тик).
    let yaw = bot.entity?.yaw ?? clientYaw ?? 0;
    ...
    // WASD без look: цель за спиной → behind (сессия перестроит).
    // lookSteer: идём с доворотом головы — behind не нужен.
    if (!lookSteer && facingDot(yaw, start, target) < -0.2) {
        clearMoveControls(bot);
        log?.(
    ...
    if (lookSteer) {
        aimYaw = pickOpenSteerYaw(bot, start, target, cells, yaw)
            ?? yawTo(start.x, start.z, target.x, target.z);
        // без мгновенного snap на весь Δyaw — первые шаги GCD стоя
        for (let i = 0; i < 6; i++) {
            const err0 = normalizeAngle(aimYaw - yaw);
            if (Math.abs(err0) <= YAW_DEADBAND_RAD) break;
            const units = Math.min(12, Math.floor(Math.abs(err0) / LOOK_GCD_STEP));
            if (units < 1) break;
            yaw = stepAngle(yaw, Math.sign(err0) * units);
            await sendLook(bot, yaw, pitch);
            await sleep(TICK_MS);
        }
    }
    ...
    while (Date.now() < deadline) {
        ...
        // 502/504: W + мягкий look (GCD-шаги), ранний увод от стены
        if (lookSteer) {
            const open = pickOpenSteerYaw(bot, { x, y, z }, target, cells, yaw);
            if (open != null) aimYaw = open;
            else aimYaw = yawTo(x, z, target.x, target.z);

            const err = normalizeAngle(aimYaw - yaw);
            if (Math.abs(err) > YAW_DEADBAND_RAD) {
                // ~0.45–0.9 rad/s целыми шагами мыши — не force-snap в точку
                const maxUnits = rndInt(8, 16);
                const remainUnits = Math.floor(Math.abs(err) / LOOK_GCD_STEP);
                const units = Math.min(maxUnits, Math.max(1, remainUnits));
                yaw = stepAngle(yaw, Math.sign(err) * units);
                if (Math.random() < 0.18) {
                    const next = pitch + (Math.random() < 0.5 ? -1 : 1) * LOOK_GCD_STEP * rndInt(1, 3);
                    if (next >= -0.35 && next <= 0.12) pitch = next;
                }
                await sendLook(bot, yaw, pitch);
            } else if (Math.random() < 0.08) {
                const next = pitch + (Math.random() < 0.5 ? -1 : 1) * LOOK_GCD_STEP * rndInt(1, 2);
                if (next >= -0.35 && next <= 0.12) {
                    pitch = next;
                    await sendLook(bot, yaw, pitch);
                }
            }
            ...
            const wallNear = wallAhead(bot, yaw, { cells, dist: 0.9, dir });
            const wallFar = wallAhead(bot, yaw, { cells, dist: 1.7, dir });
            ...
            if (wallNear) {
                ...
                const dodge = pickOpenSteerYaw(bot, { x, y, z }, target, cells, yaw);
                if (dodge == null || wallDodgeN > 14) {
                    clearMoveControls(bot);
                    log?.(
                        `walk → стена (${wallNear.x},${wallNear.z}) [${wallNear.why || '?'}]`
                ...
                if (Date.now() - lastLogAt > 2000) {
                    lastLogAt = Date.now();
                    log?.(
                        `walk → увод от стены ~(${x.toFixed(1)},${z.toFixed(1)})`
                        + ` → (${target.x.toFixed(1)},${target.z.toFixed(1)})`,
                    );
                }
                await sleep(TICK_MS);
            ...
            if (wallFar) {
                // заранее крутим к открытому курсу, W пока держим
                const early = pickOpenSteerYaw(bot, { x, y, z }, target, cells, yaw);
                if (early != null) aimYaw = early;
            } else {
                wallDodgeN = 0;
            }
            ...
            if (Date.now() - lastLogAt > 2000) {
                ...
                log?.(
                    ...
                    + ` d=${dist.toFixed(1)} keys=forward lookSteer off=${off.toFixed(1)}`,
                );
            }
            try {
                for (const key of KEY_PREF) {
                    bot.setControlState(key, key === 'forward');
                }
                bot.refreshPlayerInput?.();
                lastKeys = 'forward';
            } catch {
...
/**
 * Одна нога: WASD к точке; lookSteer → W+look.
 */
...
/**
 * Дневная прогулка как wire-запись пилота: длинный continuous W + look GCD,
 * без нарезки по вейпоинтам / soft_spin. 502/504.
 */
async function walkPilotStyleSession(bot, route, {
    shouldAbort,
    log: logFn,
    rng = Math.random,
    zones = NO_ZONES,
    ...
    const budget = (cfg?.cycleDistMin ?? 60)
        + rng() * Math.max(10, (cfg?.cycleDistMax ?? 150) - (cfg?.cycleDistMin ?? 60));
    // sell budget ~45с; запись держала W ~20с — 1–2 холда
    const deadline = Date.now() + Math.min(Number(route?.maxMs) || 50_000, 42_000);
    logFn?.(
        `walk → pilot-style continuous W budget~${budget.toFixed(0)}м (как запись, без вейпоинтов)`,
    );
    ...
    while (walkedTotal < budget - 2 && Date.now() < deadline - 2500) {
        if (typeof shouldAbort === 'function' && shouldAbort()) {
            return { ok: false, reason: 'abort', stop, legs };
        }
        if (!bot?.entity) return { ok: false, reason: 'gone', stop, legs };

        const zone = zones?.blocks?.(bot.entity.position);
        if (zone) {
            logFn?.(`walk → стоп: зона «${zone}»`);
            stop = { ...bot.entity.position };
            return {
                ok: walkedTotal >= 8,
                reason: walkedTotal >= 8 ? 'zone' : 'in_zone',
                stop,
                legs,
            };
        }

        const remain = budget - walkedTotal;
        const remMs = deadline - Date.now();
        // один холд как в записи: ~28–55 блоков / до ~22с
        const legCap = Math.min(55, Math.max(16, remain));
        const legMin = Math.min(legCap, Math.max(12, legCap * 0.7));
        const w = await antiAFKMoveForwardLook(
            bot,
            logFn,
            () => (typeof shouldAbort === 'function' && shouldAbort()) || Date.now() >= deadline,
            {
                totalDistMin: legMin,
                totalDistMax: legCap,
                sessionMsMax: Math.min(22_000, remMs),
                zones,
                label: 'walk',
            },
        );
        legs += 1;
        walkedTotal += w;
        if (bot.entity?.position) stop = { ...bot.entity.position };
        if (w < 2.5) break;
        // короткая пауза между холдами (release → снова press), как живой
        const pause = 180 + Math.floor(rng() * 520);
        const paused = await sleepAbortable(pause, bot, shouldAbort);
        if (paused) return { ok: false, reason: paused, stop, legs };
    }

    const ok = walkedTotal >= Math.min(20, budget * 0.35) || (legs >= 1 && walkedTotal >= 12);
    logFn?.(
        `walk → pilot-style конец walked=${walkedTotal.toFixed(0)}/${budget.toFixed(0)}`
        + ` legs=${legs} → (${stop.x.toFixed(0)}, ${stop.z.toFixed(0)})`,
    );
    return { ok, reason: ok ? 'ok' : 'short', stop, legs };
}
...
async function walkShopMapSession(bot, route, {
    ...
    // origin.y — якорь скана/варпа; flatY иногда залипает на другом этаже (69 vs 89).
    const originY = cached?.origin?.y;
    const flatY = cached?.flatY;
    let flatYHint = flatY ?? originY;
    if (
        Number.isFinite(originY)
        && Number.isFinite(flatY)
        && Math.abs(flatY - originY) > 8
    ) {
        flatYHint = originY;
    }
    // Клиентская физика иногда уезжает на Y≈1 при shop flatY≈70 — доскан там бесполезен.
    if (
        Number.isFinite(flatYHint)
        && Math.abs(pos0.y - flatYHint) > 12
    ) {
        const rescueName = randomWarpName();
        logFn?.(
            `walk → Y desync (y=${pos0.y.toFixed(0)} flatY=${Number(flatYHint).toFixed(0)}) → /warp ${rescueName}`,
        );
        const wait = await doEntryWarp(
            bot, `/warp ${rescueName}`, cfg.entryWarpWaitMs, shouldAbort, logFn, 'y-desync',
        );
        ...
        if (
            !pos1
            || (Number.isFinite(flatYHint) && Math.abs(pos1.y - flatYHint) > 12)
        ) {
            logFn?.(`walk → Y desync после /warp ${rescueName}`);
            return { ok: false, reason: 'y_desync' };
        }
    }

    const warpSpec = resolveEntryWarp(cfg, { username: key, anarchy });
    const whyNeed = warpSpec ? entryWarpReason(bot.entity?.position || pos0, cached, zones) : null;
    ...
    // 502/504: дневная прогулка = continuous W+look как запись (не shop_map вейпоинты)
    const anNum = Number(anarchy);
    if (anNum === 502 || anNum === 504) {
        shopSessions.delete(key);
        return walkPilotStyleSession(bot, route, {
            shouldAbort,
            log: logFn,
            rng,
            zones,
    ...
    // ушёл за край известного — доскан на чужой Y/XZ часто минуты и бесполезен
    // (особенно после «Здесь нет команд!» / physics desync). Сразу rescue-warp.
    if (!isOnShopFlatMap(map.cells, bot.entity.position, { maxDist: 16 })) {
        const fy = map.flatY ?? map.origin?.y;
        const yBad = Number.isFinite(fy) && Math.abs(bot.entity.position.y - fy) > 12;
        logFn?.(
            yBad
                ? `walk → вне сетки + Y desync (y=${bot.entity.position.y.toFixed(0)}), rescue warp`
                : 'walk → вне сетки, без доскана → rescue warp',
        );
    }
    if (!bot?.entity) return { ok: false, reason: 'gone' };
    if (!isOnShopFlatMap(map.cells, bot.entity.position, { maxDist: 16 })) {
        ...
        const rescueName = randomWarpName();
        const rescue = yBad
            ? { cmd: `/warp ${rescueName}`, name: rescueName }
            : resolveEntryWarp(cfg, { username: key, anarchy });
        if (rescue) {
            shopSessions.delete(key);
            const wait = await doEntryWarp(
                bot, rescue.cmd, Math.min(cfg.entryWarpWaitMs, 5000), shouldAbort, logFn,
                yBad ? 'y-desync rescue' : 'некуда ступить',
            );
            if (wait) return { ok: false, reason: wait };
            try {
                onWarp?.(rescue.name);
            ...
            // после варпа — только кэш, без тяжёлого dump на чужой позиции
            try {
                map = await ensureShopFlatMap(bot, {
                    radiusXZ: cfg.radiusXZ,
                    minFlatCells: cfg.minFlatCells,
                    maxAgeMs: cfg.maxAgeMs,
                    rescanDist: 1e9, // не триггерить expand
                    maxCells: cfg.maxCells,
                    zones,
                    log: logFn,
                });
    ...
    const opts = {
        ...
        zones,
        cells: map.cells,
        // lookSteer-анки (502/504) уходят в walkPilotStyleSession выше
        lookSteer: false,
    };
    ...
    let wallFix = 0;
    // unshift «крюка» без шага = busy-loop
    let hookUnshifts = 0;
    let lastHookLogAt = 0;
    let lastWallLogAt = 0;
    let spinWithoutStep = 0;
    /**
     * стена↔крюк: короткий шаг к крюку сбрасывал счётчики → вечный #1.
     * softSpin только от метража ≥3м; каждый крюк — один раз за цикл.
     */
    let softSpin = 0;
    ...
    const noteNoStep = async (why = '') => {
        softSpin += 1;
        spinWithoutStep += 1;
        if (travelled - softSpinAtTravel >= 3) {
            softSpin = 0;
            softSpinAtTravel = travelled;
            sameWallHits = 0;
            lastWallSig = '';
            wallFix = 0;
            // hookUnshifts / usedHooks НЕ сбрасываем — крюк одноразовый за цикл
        }
    ...
    const finish = (reason) => {
        ...
        // застряли в осцилляции — следующий цикл с новым планом/варпом
        if (!ok && (reason === 'soft_spin' || reason === 'spin' || reason === 'hook_loop')) {
            shopSessions.delete(key);
        }
        logFn?.(
    ...
    try {
        while (travelled < budget) {
            ...
            if (facingDot(yawNow, before, waypoint) < 0.15) {
                ...
                const hookKey = via
                    ? `${Math.round(via.x)},${Math.round(via.z)}`
                    : '';
                if (hookKey && usedHooks.has(hookKey)) {
                    session.remaining.shift();
                    logFn?.(`walk → крюк уже был (${hookKey}), снимаю точку`);
                    if (await noteNoStep('hook_used')) return finish('soft_spin');
                    continue;
                }
                if (
                    via
            ...
            // стена: максимум один replan, иначе снимаем — иначе стена↔крюк вечно
            if (!segmentWalkable(map.cells, before, waypoint)) {
                ...
                if (wallFix > 1 || sameWallHits >= 2) {
                    session.remaining.shift();
                    wallFix = 0;
                    sameWallHits = 0;
                    lastWallSig = '';
                    const now = Date.now();
                    if (now - lastWallLogAt > 4000) {
                        lastWallLogAt = now;
                        logFn?.('walk → стена: снимаю точку (без повторного обхода)');
                    }
                    if (await noteNoStep('wall_drop')) return finish('soft_spin');
                    continue;
                }
                const back = replanLeg(map.cells, before, waypoint, { blocked });
            ...
            if (!failed) {
                const legDist = xzDist(before.x, before.z, after.x, after.z);
                session.remaining.shift();
                legsDone += 1;
                spinWithoutStep = 0;
                // крошечный шаг к крюку не считается прогрессом — softSpin живёт
                if (legDist >= 2.5) {
                    softSpin = 0;
                    softSpinAtTravel = travelled;
                    hookUnshifts = 0;
            ...
            if (failed === 'behind' || failed === 'need_portal') {
                ...
                const hookKey = via ? `${Math.round(via.x)},${Math.round(via.z)}` : '';
                if (
                    via
                    && viaDot >= 0.2
                    && hookUnshifts < 2
...
export async function walkRandomRouteStop(bot, {
    ...
    const opts = {
        ...
        log: logFn,
        zones,
        lookSteer: [502, 504].includes(Number(anarchy)),
    };
