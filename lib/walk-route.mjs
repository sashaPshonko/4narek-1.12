...
async function walkShopMapSession(bot, route, {
    ...
    if (
        ...
        logFn?.(
            `walk → Y desync (y=${pos0.y.toFixed(0)} flatY=${Number(flatYHint).toFixed(0)}) → /warp shop`,
        );
        const wait = await doEntryWarp(
            bot, '/warp shop', cfg.entryWarpWaitMs, shouldAbort, logFn, 'y-desync',
        );
        ...
        if (
            ...
            logFn?.('walk → Y desync после /warp shop');
    ...
    if (!isOnShopFlatMap(map.cells, bot.entity.position, { maxDist: 16 })) {
        ...
        logFn?.(
            yBad
                ? `walk → вне сетки + Y desync (y=${bot.entity.position.y.toFixed(0)}), /warp shop`
