    ...
    try {
        while (travelled < budget) {
            ...
            if (facingDot(yawNow, before, waypoint) < 0.15) {
                ...
                if (via && xzDist(via.x, via.z, before.x, before.z) > 1.2) {
            ...
            if (failed === 'behind' || failed === 'need_portal') {
                ...
                if (via) {
                    session.remaining.unshift(via);
                    logFn?.(`walk → behind → крюк (${via.x.toFixed(1)},${via.z.toFixed(1)})`);
                    continue;
                }
                ...
                if (!session.remaining.length) {
                    ...
                    logFn?.(`walk → behind, осталось впереди ${session.remaining.length} точек`);
                }
                continue;
            }
