...
export function loadWalkRoute(filePath = WALK_ROUTE_PATH) {
    const fallback = {
         ...
         * - "random" — после preWalkChat генерим уникальный маршрут вокруг бота
         */
        mode: 'fixed',
        random: {
            legsMin: 8,
    ...
    try {
        ...
        return {
            ...
            mode: j.mode === 'random' ? 'random' : 'fixed',
...
export async function walkRandomRouteStop(bot, {
    ...
    if (route.mode !== 'random' && route.points.length < 2) {
    ...
    // random+/warp: не тащим /spawn — иначе снесёт с shop
    ...
    const total = routeTotalLength(pts, route.mode === 'random' ? false : route.loop);
    if (total < 2) return { ok: false, reason: 'route_too_short' };

    const loop = route.mode === 'random' ? false : route.loop;
    const frac =
