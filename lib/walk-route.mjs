...
import { ensureShopFlatMap, generateSpreadMapRoute, isOnShopFlatMap, loadShopMap } from './shop-map.mjs';
...
async function walkStraightToPoint(bot, target, {
    ...
    let lastLogAt = 0;
    let yaw = clientYaw ?? bot.entity?.yaw ?? yawTo(ax, az, target.x, target.z);
    ...
    while (Date.now() < deadline) {
        ...
        // факт смещения vs куда смотрим: если идём «затылком» — разворот на π
        if (!moonwalkChecked && Date.now() - walkStartedAt > 400) {
            ...
            if (mlen > 0.7) {
                ...
                if (dot < -0.2) {
                    ...
                    log?.(`walk → moonwalk fix Δyaw+180° (dot=${dot.toFixed(2)})`);
                }
...
async function walkOneLeg(bot, target, opts) {
    ...
    // свой yaw к цели — entity.yaw на FT часто отстаёт / врёт
    const yaw = bot?.entity
        ? yawTo(bot.entity.position.x, bot.entity.position.z, target.x, target.z)
        : 0;
    try {
        await bot.look(yaw, bot.entity?.pitch ?? -0.16, true);
    } catch {
        /* ignore */
    }
...
export async function walkRandomRouteStop(bot, {
    ...
    if (route.mode === 'shop_map') {
        try {
            // карта с диска (если есть) — решить, нужен ли стартовый /warp
            let cached = loadShopMap();
            const onMap = cached?.cells
                && isOnShopFlatMap(cached.cells, bot.entity.position, {
                    maxDist: route.map.onMapMaxDist,
                });

            if (!onMap && route.map.entryWarp) {
                logFn?.(
                    `walk → entryWarp ${route.map.entryWarp} (не на плоскости shop)`,
                );
                try {
                    bot.chat(route.map.entryWarp);
                } catch {
                    /* ignore */
                }
                const deadline = Date.now() + route.map.entryWarpWaitMs;
                while (Date.now() < deadline) {
                    if (typeof shouldAbort === 'function' && shouldAbort()) {
                        return { ok: false, reason: 'abort' };
                    }
                ...
                logFn?.('walk → уже на shop-плоскости, без /warp');
            }

            const map = await ensureShopFlatMap(bot, {
                radiusXZ: route.map.radiusXZ,
                minFlatCells: route.map.minFlatCells,
                maxAgeMs: route.map.maxAgeMs,
                log: logFn,
            });
            if (typeof shouldAbort === 'function' && shouldAbort()) {
                return { ok: false, reason: 'abort' };
            }
            // после варпа/дампа всё ещё не на сетке — мягкий фейл
            if (!isOnShopFlatMap(map.cells, bot.entity.position, {
                maxDist: Math.max(route.map.onMapMaxDist, 8),
            })) {
                logFn?.('walk → shop_map: бот вне сетки после entry');
                return { ok: false, reason: 'off_map' };
            }

            pts = generateSpreadMapRoute(map.cells, bot.entity.position, {
                username: username || bot.username || '',
                legsMin: route.map.legsMin,
                legsMax: Math.max(route.map.legsMin, route.map.legsMax),
                goalDistMin: route.map.goalDistMin,
                goalDistMax: Math.max(route.map.goalDistMin, route.map.goalDistMax),
            ...
            useLoop = false;
            if (pts.length < 2) {
                logFn?.('walk → shop_map route слишком короткий');
                return { ok: false, reason: 'no_route' };
            }
        } catch (err) {
