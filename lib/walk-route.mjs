 ...
 */
async function walkShopMapSession(bot, route, { shouldAbort, log: logFn, rng, username }) {
    const cfg = route.map;
    ...
    const newRoute = (why) => {
        ...
        const session = {
            ...
            obstacles: shopSessions.get(key)?.obstacles || [],
        };
    ...
    logFn?.(
        ...
        + ` (пройдено ${session.walkedTotal.toFixed(0)}м)`,
    );
    ...
    try {
        while (travelled < budget) {
            ...
            for (let j = 0; j < lookAhead; j++) {
                const detour = replanLeg(map.cells, pos, session.remaining[j], {
                    blocked,
                    simplifyEpsilon: 0.9,
                });
...
export async function walkRandomRouteStop(bot, {
    ...
    if (route.mode === 'shop_map') {
        return walkShopMapSession(bot, route, { shouldAbort, log: logFn, rng, username });
    }
