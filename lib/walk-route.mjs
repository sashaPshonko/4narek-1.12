...
export function loadWalkRoute(filePath = WALK_ROUTE_PATH) {
    const fallback = {
        ...
        map: {
            ...
            goalDistMin: 30,
            goalDistMax: 110,
            /** Только если бот не на спавне / в запретной зоне — не когда ушёл далеко */
 ...
 * Предпочтение: forward (+strafe) > strafe > back.
 * Если к цели остаётся только back — need_portal.
 */
async function walkStraightToPoint(bot, target, {
    ...
    while (Date.now() < deadline) {
        ...
        const usefulOpen = useful.length
            ? useful
            : scored.filter((s) => s.score >= DOT_KEEP); // стена на всех — пусть stuck/blocked сработает

        if (!usefulOpen.length) {
            // цель строго сзади / сбоку-сзади без полезных клавиш
            const back = scored.find((s) => s.key === 'back');
            if (back && back.score >= DOT_KEEP) {
                clearMoveControls(bot);
                log?.(
                    `walk → к цели только назад (score=${back.score.toFixed(2)})`
                    + ` → portal ~(${x.toFixed(1)},${z.toFixed(1)})`,
                );
                return { status: 'need_portal', yaw };
            }
        ...
        // только back среди полезных — не идём спиной, варпаемся
        const nonBack = usefulOpen.filter((s) => s.key !== 'back');
        if (!nonBack.length) {
            clearMoveControls(bot);
            log?.(
                `walk → доступен только back → portal`
                + ` ~(${x.toFixed(1)},${z.toFixed(1)})`,
            );
            return { status: 'need_portal', yaw };
        }

        // преимущественно вперёд: forward если полезен, иначе лучший strafe
        const pick = new Set();
        const fwdOk = nonBack.find((s) => s.key === 'forward');
        if (fwdOk) {
            pick.add('forward');
            // лёгкий strafe, если сильно уводит в сторону
            ...
            if (side && side.score >= 0.35) pick.add(side.key);
        } else {
            const best = nonBack.sort((a, b) => b.score - a.score)[0];
            pick.add(best.key);
        }

        // стена прямо по выбранному направлению движения
        ...
        if (!pick.size) {
            // после фильтра стен снова только back?
            const again = usefulOpen.filter(
                (s) => s.key !== 'back' && !wallAhead(bot, yaw, { cells, dir: keyDirXZ(yaw, s.key) }),
            );
            if (!again.length) {
                clearMoveControls(bot);
                log?.('walk → вперёд/strafe закрыты стеной, back не берём → portal');
                return { status: 'need_portal', yaw };
            }
 ...
 * «Ушёл далеко от последнего скана» — не причина: иначе /warp portal
...
async function walkShopMapSession(bot, route, { shouldAbort, log: logFn, rng, username, zones }) {
    ...
    const whyWarp = cfg.entryWarp ? entryWarpReason(pos0, cached, zones) : null;
    if (whyWarp === 'запретная зона' && !cfg.entryWarp) {
        logFn?.(`walk → бот в зоне «${zones.blocks(pos0)}», а варпа нет — стою`);
        return { ok: false, reason: 'in_zone' };
    }
    if (whyWarp) {
        const wait = await doEntryWarp(
            bot, cfg.entryWarp, cfg.entryWarpWaitMs, shouldAbort, logFn, whyWarp,
        );
    ...
    if (!isOnShopFlatMap(map.cells, bot.entity.position, { maxDist: 16 })) {
        ...
        if (cfg.entryWarp) {
            shopSessions.delete(key);
            const wait = await doEntryWarp(
                bot, cfg.entryWarp, cfg.entryWarpWaitMs, shouldAbort, logFn, 'некуда ступить',
            );
            if (wait) return { ok: false, reason: wait };
            try {
                map = await ensureShopFlatMap(bot, {
    ...
    try {
        while (travelled < budget) {
            ...
            // к цели только назад / forward+strafe закрыты — варп portal, план сброс
            if (failed === 'need_portal') {
                shopSessions.delete(key);
                if (cfg.entryWarp) {
                    const wait = await doEntryWarp(
                        bot, cfg.entryWarp, cfg.entryWarpWaitMs, shouldAbort, logFn, 'только назад',
                    );
                    if (wait) return { ok: false, reason: wait, stop, legs: legsDone };
                }
                return finish(legsDone > 0 ? 'portal_reset' : 'need_portal');
            }
...
export async function walkRandomRouteStop(bot, {
    ...
    try {
        for (let leg = 0; leg < targets.length; leg++) {
            ...
            if (res === 'need_portal') {
                const cmd = route.map?.entryWarp || '/warp portal';
                const waitMs = route.map?.entryWarpWaitMs || 8000;
                const wait = await doEntryWarp(
                    bot, cmd, waitMs, shouldAbort, logFn, 'только назад',
                );
                if (wait) return { ok: false, reason: wait, stop: lastStop, legs: done };
                return finishPartial('need_portal');
            }
            if (res === 'zone') {
