 ...
 * preWalkChat (напр. /spawn) — временный костыль, выключается пустым полем в JSON.
...
export async function walkRandomRouteStop(bot, {
    ...
    // если не на точке — сначала дойти до ближайшей вершины, дуга считается от неё
    const stop = pointAlongRoute(pts, fromIdx, stopDist, route.loop);
    if (!stop) return { ok: false, reason: 'bad_stop' };

    const targets = [];
    if (!near.onPoint) {
        targets.push(pts[fromIdx]);
    }
