...
/**
 * Стена прямо по курсу: карта не содержит клетки ИЛИ blockAt — твёрдый
 * столб на высоте тела. Ловим ДО касания, а не после 2с «вдавливания».
 */
function wallAhead(bot, yaw, { cells = null, dist = 0.68 } = {}) {
...
async function walkStraightToPoint(bot, target, {
    ...
    while (Date.now() < deadline) {
        ...
        const wall = wallAhead(bot, yaw, { cells });
        if (wall) {
            clearMoveControls(bot);
            log?.(
...
function entryWarpReason(pos, cached, zones) {
    ...
    const near = nearestCell(cached.cells, pos.x, pos.z);
    if (!near) return 'нет карты';
    // другой этаж / другая локация, не «далеко по xz»
    if (Math.abs((near.y ?? pos.y) - pos.y) > 18) return 'другая высота';
    return null;
}
