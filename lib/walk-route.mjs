...
async function walkToPoint(bot, target, opts) {
    ...
    while (Date.now() < deadline) {
        ...
        if (moved > 0.08) {
            ...
            lastZ = z;
            lastMoveAt = Date.now();
        } else if (Date.now() - lastMoveAt > 3500) {
        ...
        const wantYaw = yawTo(x, z, target.x, target.z);
        const ahead = aheadBodyName(bot, bot.entity.yaw, 1.05);
        const blocked = isBlockingName(ahead);
        // На короткой дистанции слайд уводит в чужие карманы — только nudge.
        ...
        const yawErr = await stepLookToward(bot, lookX, lookZ, maxYawRadPerSec, TICK_MS / 1000);
        try {
            bot.setControlState('left', sliding && slideSide === 'left');
            bot.setControlState('right', sliding && slideSide === 'right');
            if (Math.abs(yawErr) > 0.7 && !sliding) {
                bot.setControlState('forward', false);
                bot.setControlState('sprint', false);
            } else {
                bot.setControlState('forward', true);
                bot.setControlState('sprint', !sliding && dist > 5 && Math.abs(yawErr) < 0.22);
            }
        } catch {
            /* ignore */
        }
...
export async function walkRandomRouteStop(bot, {
    ...
    // Барьер: с z≳21 на юг к спавну не идут (air, но XZ не меняется).
    // На плазе — только вдоль трека вперёд, без spawn0 назад.
    const onPlaza = pos.z >= 19.5;
    let entry;
    let prependStart = false;
    if (onPlaza) {
        if (vertex.dist <= route.arriveXZ * 2.5) {
            entry = { ...vertex, dist: vertex.dist, via: 'on_vertex' };
        } else if (onTrack.dist <= 10) {
            entry = onTrack.dist <= route.arriveXZ * 2.2
                ? { ...onTrack, via: 'projection' }
                : { ...vertex, dist: vertex.dist, via: 'vertex' };
        } else {
            logFn?.(
                `walk → pos (${pos.x.toFixed(1)},${pos.y.toFixed(1)},${pos.z.toFixed(1)}) `
                + `плаза далеко от трека (${onTrack.dist.toFixed(1)}) — skip`,
            );
            return { ok: false, reason: 'too_far', entry: onTrack };
        }
    } else if (vertex.dist <= route.arriveXZ * 2.5) {
        entry = { ...vertex, dist: vertex.dist, via: 'on_vertex' };
    ...
    } else if (onTrack.dist <= route.maxEntryDist) {
