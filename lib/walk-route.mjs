...
/**
 * К цели: по умолчанию WASD без look; lookSteer (502/504) — W + look к цели.
...
async function walkStraightToPoint(bot, target, {
    ...
    let lastLogAt = 0;
    // WASD: курс фиксируем. lookSteer: yaw обновляем к цели каждый тик.
    ...
    // lookSteer: просто разворачиваем голову к цели и идём.
    ...
    if (lookSteer) {
        yaw = yawTo(start.x, start.z, target.x, target.z);
        try {
            await bot.look(yaw, bot.entity?.pitch ?? 0, true);
            if (bot.entity) bot.entity.yaw = yaw;
        } catch {
            /* ignore */
        }
    }
    ...
    while (Date.now() < deadline) {
        ...
        // 502/504: W + look к цели (как pilot), без strafe-рывков
        if (lookSteer) {
            const wantYaw = yawTo(x, z, target.x, target.z);
            yaw = wantYaw;
            try {
                await bot.look(wantYaw, bot.entity?.pitch ?? 0, true);
                if (bot.entity) bot.entity.yaw = wantYaw;
            } catch {
                /* ignore */
            }
            if (wallAhead(bot, yaw, { cells, dir: lookXZ(yaw) })) {
                clearMoveControls(bot);
                const wall = wallAhead(bot, yaw, { cells, dir: lookXZ(yaw) });
                log?.(
