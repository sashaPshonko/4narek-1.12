...
let pingTimer = null;
/** Экспоненциальный backoff при мёртвом GUI host (иначе спам handshake timeout каждые ~8с). */
...
function onMessage(raw) {
    ...
    if (type === 'staff_live' || type === 'staff_lock') {
        const peer = String(msg.peer || '').trim();
        if (peer) console.log(`[gui-desk] ${type} checker=${peer}`);
        return;
    }
    if (type === 'session_end') {
...
function noteGuiFail(err) {
    reconnectAttempt += 1;
    const now = Date.now();
    if (now - lastFailLogAt < FAIL_LOG_EVERY_MS) return;
    lastFailLogAt = now;
    const msg = err?.message || err || 'fail';
    console.warn(
        `[gui-desk] ${msg} (попытка ${reconnectAttempt}, next ~${Math.round(reconnectDelayMs() / 1000)}с)`,
    );
}
