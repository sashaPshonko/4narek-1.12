...
function isBanChatText(text) {
    // FunTime: «ВЫ ЗАБАНЕНЫ!» / «Вы были кикнуты … ВЫ ЗАБАНЕНЫ!»
...
export async function pingClanOwner({ username, password, proxyString, anarchy, log = console.log }) {
    ...
    return new Promise((resolve) => {
        ...
        const onChatText = (raw) => {
            ...
            // бан на анке приходит чатом (не кик-причиной): «ВЫ ЗАБАНЕНЫ!»
            if (isBanChatText(text)) {
                plog(`ban chat: ${text.slice(0, 220)}`);
                clearTimeout(timer);
        ...
        bot.on('kicked', (reason) => {
            ...
            if (settled) return;
            // если в тексте кика всё же бан-чат — тоже считаем
            if (isBanChatText(text)) {
                clearTimeout(timer);
                done({ status: 'banned', reason: text.slice(0, 500) });
                return;
        ...
        bot.once('spawn', async () => {
            ...
            if (!settled) {
                clearTimeout(timer);
                if (joinedAnarchy) done({ status: 'ok' });
                else {
                    done({
                        status: 'error',
