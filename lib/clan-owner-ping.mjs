...
function isBanChatText(text) {
    return String(text || '').toLowerCase().includes('вы забанены');
}
...
export async function pingClanOwner({ username, password, proxyString, log = console.log }) {
    ...
    return new Promise((resolve) => {
        ...
        bot.on('kicked', (reason) => {
            ...
            // Бан определяем по чату «вы забанены», не по кику.
            // Кик без чата — просто обрыв (уже онлайн / vpn / etc).
            if (settled) return;
            clearTimeout(timer);
        ...
        bot.on('messagestr', (msg) => {
            ...
            // как 4narek-old: бан = чат «Вы забанены…»
            if (isBanChatText(text)) {
                plog(`ban chat: ${text.slice(0, 200)}`);
                clearTimeout(timer);
